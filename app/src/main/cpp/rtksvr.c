...
static int decoderaw(rtksvr_t *svr, int index)
{
    ...
    for (i=0;i<svr->nb[index];i++) {
        ...
        /* input rtcm/receiver raw data from stream从流输入rtcm /接收器原始数据 */
        ...
        else if (svr->format[index]==STRFMT_RTCM3) {                                     //解码观测数据流o文件
...
#endif
{
    ...
    for (cycle=0;svr->state;cycle++) {
        ...
        for (i=0;i<3;i++) {
            ...
            /* read receiver raw/rtcm data from input stream读取数据流信息，先做判断 */
            if ((n=strread(svr->stream+i,p,q-p))<=0) {      /*n的值是什么，得考虑下*/
                continue;
            }
            /* write receiver raw/rtcm data to log stream 将接收器raw / rtcm数据写入日志流*/
            strwrite(svr->stream+i+5,p,n);
            svr->nb[i]+=n;
            
            /* save peek buffer 保存peek缓冲区*/
            rtksvrlock(svr);
            n=n<svr->buffsize-svr->npb[i]?n:svr->buffsize-svr->npb[i];/*//取小的值*/
        ...
        for (i=0;i<3;i++) {
            if (svr->format[i]==STRFMT_SP3||svr->format[i]==STRFMT_RNXCLK) {
                /* decode download file 文件形式解码*/
                decodefile(svr,i);
            }
            else {
                /* decode receiver raw/rtcm data 二进制形式解码*/
        ...
        //liuyi差分需要用到
        /* averaging single base pos 基站平均单点位置*/
        if (fobs[1]>0&&svr->rtk.opt.refpos==POSOPT_SINGLE) {
            if ((svr->rtk.opt.maxaveep<=0||svr->nave<svr->rtk.opt.maxaveep)&&
                ...
                svr->nave++;
                for (i=0;i<3;i++) {
                    svr->rb_ave[i]+=(sol.rr[i]-svr->rb_ave[i])/svr->nave;        //平均基准站位置
                }
            }
            for (i=0;i<3;i++) svr->rtk.opt.rb[i]=svr->rb_ave[i];
        }
        /* for each rover observation data为每个流动站观测数据 */
        //加手机观测值

//        for (i=0;i<fobs[0];i++) {                                 //edit by liuyi 原函数
        //只执行一次，
        for (i=0;i<1;i++) {
            obs.n=0;
//           for (j=0;j<svr->obs[0][i].n&&obs.n<MAXOBS*2;j++) {      //edit by liuyi 原函数
//                obs.data[obs.n++]=svr->obs[0][i].data[j];
//            }

            //memset(&obs,0x00, sizeof(obs));
            //加手机观测值放到obs.data的前边，只用了L1观测值ObsData.GpsL1_t[j]
            for(j = 0;j<ObsData.gpsSvNum&&obs.n<MAXOBS*2;j++) {

                gtime_t gpstTemp = gpst2time(ObsData.GPSTweek[j],ObsData.GPSTsecond[j]);
                obs.data[obs.n].time.time = gpstTemp.time;
                obs.data[obs.n].time.sec = gpstTemp.sec;
                obs.data[obs.n].P[0] = ObsData.GpsC1_t[j];
                obs.data[obs.n].L[0] = ObsData.GpsC1_t[j]/0.1902936727984;
                obs.data[obs.n].D[0] = ObsData.GpsD1_t[j];
                obs.data[obs.n].sat = (unsigned char)satno(SYS_GPS,ObsData.Gpsprn_t[j]);
                //obs.data[obs.n].rcv = '\x01';
                obs.data[obs.n].rcv = (unsigned char)1;
               // int2char(&obs.data[obs.n].sat,ObsData.Gpsprn_t[j]);//GPS卫星号
                obs.n++;
            }
//            for(j = 0;j<ObsData.bdsSvNum&&obs.n<MAXOBS*2;j++) {
//
//                gtime_t gpstTemp = gpst2time(ObsData.BDSTweek[j],ObsData.BDSTsecond[j]);
//                obs.data[obs.n].time.time = gpstTemp.time;
//                obs.data[obs.n].time.sec = gpstTemp.sec;
...
//            }
            ...
            /* rtk positioning */
            rtksvrlock(svr);
            rtkpos(&svr->rtk,obs.data,obs.n,&svr->nav);           //单点定位
            rtksvrunlock(svr);
        ...
        /* send null solution if no solution如果没有解决方案，则发送null解 (1hz) */
        if (svr->rtk.sol.stat==SOLQ_NONE&&(int)(tick-tick1hz)>=1000) {
            writesol(svr,0);
            tick1hz=tick;
        }
        /* write periodic command to input stream将周期性命令写入输入流 */
        for (i=0;i<3;i++) {
            periodic_cmd(cycle*svr->cycle,svr->cmds_periodic[i],svr->stream+i);
        }
        /* send nmea request to base/nrtk input stream将nmea请求发送到base / nrtk输入流 */
...
/* lock/unlock rtk server锁定/解锁rtk服务器 ------------------------------------------------------
                       ...
                       solopt_t *solopt, stream_t *moni, char *errmsg)
{
    ...
    for (i=0;i<3;i++) {                         /* input/log streams */
        ...
        /* initialize receiver raw and rtcm control 初始化流，原始数据、RTCM流*/
        ...
        /* connect dgps corrections 接差分实时流*/
    ...
    /* set solution options 设置解算模式*/
    for (i=0;i<2;i++) {
        svr->solopt[i]=solopt[i];
    }
    /* set base station position 相对距离设置基准站坐标*/
    if (prcopt->refpos!=POSOPT_SINGLE) {
        for (i=0;i<6;i++) {
            svr->rtk.rb[i]=i<3?prcopt->rb[i]:0.0;
        }
    }
    /* update navigation data 更新广播星历*/
    for (i=0;i<MAXSAT *2;i++) svr->nav.eph [i].ttr=time0;
    for (i=0;i<NSATGLO*2;i++) svr->nav.geph[i].tof=time0;/*格洛纳斯广播星历*/
    for (i=0;i<NSATSBS*2;i++) svr->nav.seph[i].tof=time0;/*SBSA广播星历*/
    updatenav(&svr->nav);
    
    /* set monitor stream 设置监测站*/
    svr->moni=moni;
    
    /* open input streams 打开进入数据流*/
    for (i=0;i<8;i++) {
        rw=i<3?STR_MODE_R:STR_MODE_W;
        if (strs[i]!=STR_FILE) rw|=STR_MODE_W;
        if (!stropen(svr->stream+i,strs[i],rw,paths[i])) {        /* 打开实时数据流*/
            sprintf(errmsg,"str%d open error path=%s",i+1,paths[i]);
            for (i--;i>=0;i--) strclose(svr->stream+i);
            return 0;
        }
        /* set initial time for rtcm and raw 初始化数据流*/
        if (i<3) {
    ...
    /* sync input streams 同步数据流*/
    strsync(svr->stream,svr->stream+1);
    strsync(svr->stream,svr->stream+2);
    
    /* write start commands to input streams 将开始命令写入输入流*/
    for (i=0;i<3;i++) {
        if (!cmds[i]) continue;
    ...
    /* write solution header to solution streams 将解决方案头写入解决方案流 */
    for (i=3;i<5;i++) {
        writesolhead(svr->stream+i,svr->solopt+i-3);
    }
    /* create rtk server thread 开始解码、计算线程*/
#ifdef WIN32
...
void int2char(char *str,int num)
{
