...
{
    double x[NX]={0},dx[NX],Q[NX*NX],*v,*H,*var,sig,dop[4]={0};
    int i,j,k,info,stat,nv,ns,ii;
    int sysprn,gpsnum,glonum,bdsnum;
    //char *syst;
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (norm(dx,NX)<1E-4) {
            ...
            /* validate solution   edit by liuyi*/
                    ...
                    double *azel, int *vsat, double *resp, char *msg)
{
    ...
    for (i=0;i<n;i++) {
        ...
        /* estimate receiver position without a satellite排除卫星估计接收器位置 */
