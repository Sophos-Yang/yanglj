...
/* global variables ----------------------------------------------------------*/
static int statlevel=0;          /* rtk status output level (0:off) rtk状态输出级别*/
...
/* output solution status，输出解决方案状态 ----------------------------------------------------*/
                  ...
                  const nav_t *nav)
{
    ...
    /* satellite positions/clocks计算卫星坐标、钟差 */
    satposs(time,obs,n,nav,opt->sateph,rs,dts,var,svh);
    
    /* undifferenced residuals for base station基站的无差别残差 */
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    /* count rover/base station observations 计算流动站/基站观测*/
    ...
    /* suppress output of single solution抑制单一解决方案的输出 */
    if (!opt->outsingle) {
