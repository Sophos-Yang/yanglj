...
public class UiLogger implements GnssListener {
    ...
    static double weekNumber111=0;
    static double tRxSeconds111=0;
    @Override
    public void onGnssMeasurementsReceived(GnssMeasurementsEvent event) {
        ...
        for (int i = 0; i < rowLen; i++) {
            if(array[i][1]!="INVALID_VALUE"&&Double.parseDouble(array[i][3])>25.0) {
 //              int prn = new Integer(array[Vgpsnum][0].substring(1));
                StaticGnssData.Gpsprn[Vgpsnum] = Integer.parseInt(array[i][0].substring(1,3));
                StaticGnssData.GpsC1[Vgpsnum] = Double.parseDouble(array[i][1]);
                StaticGnssData.GpsL1[Vgpsnum] = Double.parseDouble(array[i][2]);
                StaticGnssData.GPSTweek[Vgpsnum] = (int) weekNumber111;
                StaticGnssData.GPSTsecond[Vgpsnum] = tRxSeconds111;
                Vgpsnum++;
                StaticGnssData.gpssvnum = Vgpsnum;
                Log.d(Tag, "GPS:" + " " + StaticGnssData.Gpsprn[Vgpsnum] + "  " + "Week: " + StaticGnssData.GPSTweek[Vgpsnum]
                        + " " + "Second: " + StaticGnssData.GPSTsecond[Vgpsnum]
                        + " " + "伪距: " + " " + StaticGnssData.GpsC1[Vgpsnum]
                        + " " + "载波: " + " " + StaticGnssData.GpsL1[Vgpsnum]
                        + "\n");
            }
        ...
        StaticGnssData.gpssvnum=rowLen+1;
    ...
    private String gnssClockToString(GnssClock gnssClock){
        ...
        if(gnssClock.getHardwareClockDiscontinuityCount() == -1){
           ...
           // double tRxSeconds;
            ...
            weekNumber111 = Math.floor(- (gnssClock.getFullBiasNanos() * 1e-9 / 604800));
            double weekNumberNanos = weekNumber111 * 604800 * 1e9;
            if (gnssClock.hasBiasNanos() == false) {
                tRxSeconds111 = (gnssClock.getTimeNanos() - gnssClock.getFullBiasNanos() - weekNumberNanos) * 1e-9;
            } else {
                tRxSeconds111 = (gnssClock.getTimeNanos() - gnssClock.getFullBiasNanos() - gnssClock.getBiasNanos() - weekNumberNanos) * 1e-9;
            }

            String DeviceName = Build.DEVICE;
            FileLogger.GPSWStoGPST gpswStoGPST = new FileLogger.GPSWStoGPST();
            FileLogger.ReturnValue value = gpswStoGPST.method(weekNumber111,tRxSeconds111);                 ///////转化为GPSTIME
                        ...
                        if (SettingsFragment.usePseudorangeSmoother && prm != 0.0) {
                            if (index < 300) {
                                if (SettingsFragment.usePseudorangeRate) {
                                    ...
                                    array[arrayRow][1] = String.format("%15.4f", LAST_SMOOTHED_PSEUDORANGE[index], " ", " ");
                                } else {
                                    //载波相位平滑伪距算法实现
